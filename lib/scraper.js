    ...
    if( fs.existsSync(path) ) {
        ...
        lines.map((l) => {
            if(l.length > 0) {
                ...
                let summary_key = contract.categoria + '_' + contract.subcategoria + ( (contract.hasOwnProperty('rubro')? '_' + contract.rubro : '') );
                        ...
                        for(let k=0; k<cat.subcategories.length; k++) {
                            ...
                            if(skipSubcats && startSubcat != currentSubcat) {
                                ...
                                if(subcat.hasOwnProperty('url') && subcat.url != '') {
                                    ...
                                    subcat['contracts'] = await processResultsPage(y, seenContractIDs, categorySummary, subcat, currentCat, currentSubcat);
                                }
                                    ...
                                    for(let m=0; m<subcat.rubros.length; m++) {
                                        ...
                                        if(skipRubros && startRubro != currentRubro) {
                                            ...
                                            if(rubro.hasOwnProperty('url') && rubro.url != '') {
                                                ...
                                                rubro['contracts'] = await processResultsPage(y, seenContractIDs, categorySummary, rubro, currentCat, currentSubcat, currentRubro);
                                            }
                                        }
                                    }
                                }
...
async function processResultsPage(year, seenContractIDs, categorySummary, obj, catID, subcatID, rubroID=null) {
    let contracts = [];
