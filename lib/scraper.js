                        ...
                        for(let k=0; k<cat.subcategories.length; k++) {
                            ...
                            if(skipSubcats && startSubcat != currentSubcat) {
                                ...
                                if(subcat.hasOwnProperty('url') && subcat.url != '') {
                                    ...
                                    subcat['contracts'] = await processResultsPage(y, subcat, currentCat, currentSubcat);
                                }
                                    ...
                                    for(let m=0; m<subcat.rubros.length; m++) {
                                        ...
                                        if(skipRubros && startRubro != currentRubro) {
                                            ...
                                            if(rubro.hasOwnProperty('url') && rubro.url != '') {
                                                ...
                                                rubro['contracts'] = await processResultsPage(y, rubro, currentCat, currentSubcat, currentRubro);
                                            }
                                        }
                                    }
                                }
...
async function processResultsPage(year, obj, catID, subcatID, rubroID=null) {
    let contracts = [];
    ...
    try {
        ...
        await page.waitForSelector('#detailgral');
        ...
        if(resultsOverview.total > 0 && resultsOverview.pages > 0) {
            ...
            for(let n=0; n<totalContracts; n++) {
                ...
                if(contractStatus == false) {
                    // TODO: with contractStatus, build list of which contracts failed and retry them
                }
...
async function getContractDetails(page, contract) {
    // let contractURL = 'http://compras.imss.gob.mx' + contract.url.replace('imsscomprofich', 'imsscomprofichprt');
...
function saveToFile(entities) {
    ...
    for(var y in entities) {
        if(entities.hasOwnProperty(y)){
            ...
            if(verbose) console.log('Writing file for year ' + y);
